...
from smart_invoice_app.app import get_country_code, get_settings, set_defaults_exception
...
def calculate_taxes(data, company):
    ...
    for row in data:
        ...
        item_tax_template, tax_rate = get_tax_rate(
            row.item_tax_template, company, invoice_country_code, invoice
        )
...
def get_tax_rate(item_tax_template, company, country_code, invoice):
    ...
    if not item_tax_template:
        tax_code = company.item_tax_code
        if not tax_code:
            set_defaults_exception()

        code_doc = frappe.get_cached_value(
            "Code", {"cd": tax_code}, ["name", "cd_nm", "user_dfn_cd1"], as_dict=True
        )
...
def get_columns():
    columns = [
        ...
        {"label": "TPIN", "fieldname": "tax_id", "fieldtype": "Data", "width": 115},
        {
            "label": "Customer",
            "fieldname": "customer",
            "fieldtype": "Link",
