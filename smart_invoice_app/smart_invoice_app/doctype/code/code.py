...
class Code(Document):
    ...
    def attempt_code_mapping(self, force=False):
        ...
        if map_entry:
            ...
            frappe.msgprint(
                f"Mapped {self.mapped_doctype}: {map_entry} for ZRA Code {self.cd_nm}",
                alert=True,
            )
        else:
            frappe.msgprint(
                f"Failed to auto map ZRA Code ({self.cd_nm}) to any {self.mapped_doctype}. You have to map it manually",
                alert=True,
            )
    ...
    def get_code_field(self):
        try:
            ...
            custom_fields = frappe.get_all(
                "Custom Field",
                filters={"dt": self.mapped_doctype},
                fields=["fieldname"],
                limit=0,
            )
            ...
            return next(
                (
                    ...
                    if field.fieldname.lower().endswith(("_cd", "cd", "code"))
                ),
                None,
            )
    ...
    def find_mapping_entry(self):
        if self.mapped_doctype in ["UOM"]:
            try:
                ...
                if mapped_uom:
                    ...
                    if (
                        mapped_uom[0]["custom_cd"] != self.name
                        or mapped_uom[0]["custom_code_cd"] != self.cd
                        or mapped_uom[0]["custom_code_class_name"] != self.cd_cls
                    ):
        ...
        if self.mapped_doctype in ["Mode of Payment", "Branch"]:
            ...
            frappe.msgprint(
                f"No mapping found for {self.mapped_doctype}. You have to map it manually",
                alert=True,
            )
    ...
    def find_db_entry(self, name_field, custom_field=None):
        conditions = [f"{name_field} = %s", f"LOWER({name_field}) = LOWER(%s)"]

        if custom_field:
            conditions.extend(
                [f"{custom_field} = %s", f"LOWER({custom_field}) = LOWER(%s)"]
            )
